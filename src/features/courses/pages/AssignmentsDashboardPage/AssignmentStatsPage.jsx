...
const AssignmentStatsPage = () => {
  ...
  return (
    <div className="analytics-page assignment-stats-page">
      <Link to={`/teacher/course/${courseId}/assignments`} className="back-link">
        <i className="fas fa-arrow-left"></i> Quay lại danh sách bài tập
      </Link>
      <h2 className="stats-title">Thống kê bài tập: <span className="highlight">{data.title}</span></h2>
      <div className="overview-grid">
        ...
        <div className="overview-item stat-card">
          ...
          <p className="stat-number warning">{data.averageGrade ? Number(data.averageGrade).toFixed(1) : '-'}</p>
        </div>
        <div className="overview-item stat-card">
